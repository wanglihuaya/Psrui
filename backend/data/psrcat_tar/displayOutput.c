  ...
  if (nohead==0)
      ...
      if (tbForm==1)
	{
	  for (j=0;j<np;j++)
		...
		{
		  strcat(headerStr,defParam[ip].header1);
		  sprintf(dispStr,"%%%lus",10-strlen(defParam[ip].header1));
  ...
  for (i=0;i<npsr;i++)
      ...
      for (j=0;j<np;j++)
	{
	  ...
	  if (webflag==1 && ptr[j] == 2 && dispParam.set1==1 && (tbForm==0 || tbForm==1 || tbForm==4 || tbForm==6 || tbForm==7))
		...
		printf("%s",dispParam.val);
		// George Hobbs remove for security on 4th April 2024
		...
		{
		  // GEORGE ADDED THIS APRIL 2024  
		  strcat(temp3,temp+pos);
